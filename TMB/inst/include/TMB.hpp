...
#define eigen_assert(x)                                                \
  if (!(x))                                                            \
  {                                                                    \
    eigen_REprintf("TMB has received an error from Eigen. ");          \
...
namespace TMBad
{
  ...
  using std::isfinite;
  bool isfinite(const TMBad::ad_aug &x) CSKIP({ return isfinite(x.Value()); }) using std::isinf;
  bool isinf(const TMBad::ad_aug &x) CSKIP({ return isinf(x.Value()); }) using std::isnan;
...
namespace CppAD
{
  ...
  bool Variable(double x) CSKIP({ return false; })
      ...
      using std::isfinite;
  template <class T>
  bool isfinite(const AD<T> &x) CSKIP({ return isfinite(Value(x)); }) using std::isinf;
  template <class T>
  bool isinf(const AD<T> &x) CSKIP({ return isinf(Value(x)); }) using std::isnan;
  template <class T>
